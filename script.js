...
function markInvalidFields(hasErrors) {
  form.querySelectorAll("input, select").forEach((field) => {
    ...
    if (wrap) wrap.classList.toggle("invalid", hasErrors && !field.checkValidity());
  });
}
...
form.querySelectorAll("input, select").forEach((field) => {
  field.addEventListener("input", () => {
    const wrap = field.closest(".field-wrap");
    if (wrap) wrap.classList.toggle("invalid", field.value !== "" && !field.checkValidity());
  });
});
...
function handleChatSuggestion(el) {
  const question = el.textContent.trim();
  const answers = {
    "Why is living area the top feature?": "Living area directly correlates with usable space and construction cost — the model learned it's the strongest predictor of price across all property tiers.",
    "What makes a Luxury property?": "A predicted price above ₹75,00,000 classifies a property as Luxury. This tier typically reflects large living areas, excellent condition, and premium locations.",
    "How is confidence score calculated?": "Confidence starts from the model's R² accuracy, then applies small penalties for feature values that fall far from the dataset's typical range.",
  };
  const reply = answers[question] || "That's a great question! The model uses 10 property signals to generate each valuation.";
  chatbotPanel.querySelector("p").textContent = reply;
}
